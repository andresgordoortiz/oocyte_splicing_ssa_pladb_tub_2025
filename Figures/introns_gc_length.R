...
make_and_save_introns <- function(dataset_name,
  ...
  all_introns <- bind_rows(introns_gc, unchanged_gc) %>%
    mutate(splicing = factor(splicing, levels = c("Retained","Removed","Unchanged")))
...
all_drugs_data <- bind_rows(
  ...
  differential_ssa %>%
    filter(grepl("INT", EVENT)) %>%
    left_join(dplyr::select(event_info, EVENT, Seq_A, LE_o), by = "EVENT") %>%
    mutate(splicing = if_else(deltapsi < 0, "Removed", "Retained"), drug = "ssa"),

  # TUB
  differential_tub %>%
    filter(grepl("INT", EVENT)) %>%
    left_join(dplyr::select(event_info, EVENT, Seq_A, LE_o), by = "EVENT") %>%
    mutate(splicing = if_else(deltapsi < 0, "Removed", "Retained"), drug = "tub"),

  # PLADB
  differential_pladb %>%
    filter(grepl("INT", EVENT)) %>%
    left_join(dplyr::select(event_info, EVENT, Seq_A, LE_o), by = "EVENT") %>%
    mutate(splicing = if_else(deltapsi < 0, "Removed", "Retained"), drug = "pladb")
) %>%
  mutate(gc = gc_content(Seq_A))
...
p_gc_unified <- ggplot(all_drugs_data, aes(x = splicing, y = gc, fill = splicing)) +
  ...
  scale_alpha_manual(values = c("Retained" = 0.5, "Removed" = 0.5, "Unchanged" = 0.15)) +
  scale_y_continuous(
    expand = expansion(mult = c(0.05, 0.15)),  # Add 15% space on top
    limits = c(0, 100)
  ) +
...
if(length(plots_gc) > 0 && length(plots_len) > 0) {
  ...
  combined_plot <- (plots_gc$ssa | plots_gc$tub | plots_gc$pladb) /
    ...
    plot_annotation(
      title = "Introns: GC Content and Length Across Splicing Drugs",
      theme = theme(
  ...
  # Save unified plot
  ggsave("introns_unified_combined.png", plot = combined_plot, width = 18, height = 10, dpi = 600)
  ggsave("introns_unified_combined.pdf", plot = combined_plot, device = cairo_pdf, width = 18, height = 10)
  message("  -> saved: introns_unified_combined.png and introns_unified_combined.pdf")
} else {
