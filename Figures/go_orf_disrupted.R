...
genes_for_enrichment <- unique(combined$GENE[grepl("ORF",combined$Impact)])
...
if (!is.null(ego_bp) && nrow(as.data.frame(ego_bp)) > 0) {
  p_bp <- create_enhanced_go_dotplot(
    ...
    ontology_name = "BP",
    top_n = 20,
    output_file = "GO_BP_enhanced_orf"
  )
...
if (!is.null(ego_cc) && nrow(as.data.frame(ego_cc)) > 0) {
  p_cc <- create_enhanced_go_dotplot(
    ...
    ontology_name = "CC",
    top_n = 20,
    output_file = "GO_CC_enhanced_orf"
  )
...
if (!is.null(ego_mf) && nrow(as.data.frame(ego_mf)) > 0) {
  p_mf <- create_enhanced_go_dotplot(
    ...
    ontology_name = "MF",
    top_n = 20,
    output_file = "GO_MF_enhanced_orf"
  )
