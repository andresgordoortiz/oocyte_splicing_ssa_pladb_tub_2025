...
# ---- Ensure desired facet order: Tubercidin, Pladienolide B, Spliceostatin A ----
desired_order <- c("Tubercidin", "Pladienolide B", "Spliceostatin A")
combined_df$drug <- factor(combined_df$drug, levels = desired_order)
...
volcano_plot_all <- ggplot(plot_df, aes(x = deltapsi, y = negLog10FDR_transformed)) +
  geom_point(aes(color = ifelse(significant == "sig", as.character(drug), "ns")),
             size = 2.5, alpha = 0.3, stroke = 0, show.legend = TRUE) +
...
ggsave("volcano_combined_deltapsi_facet.png",
       volcano_plot_all,
       device = png,
       width = 8,
       height = 3,
